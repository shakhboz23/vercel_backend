...
export class BotDashboardService {
  ...
  async reyting_courses(ctx: Context) {
    ...
    const buttons = [...groups.entries()].map(([groupId, title]) => [
      { text: title, callback_data: `reyting_group_${groupId}` },
    ]);

    await ctx.reply("📊 Reytingni ko'rish uchun guruhni tanlang:", {
  ...
  async reytingForGroup(ctx: Context, group_id: number) {
    ...
    const buttons = courses.map((course: any) => [
      { text: course.title, callback_data: `reyting_course_${course.id}` },
    ]);
